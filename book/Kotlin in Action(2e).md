...
- 인라인 클래스로 선언하려면, 클래스가 프로퍼티를 하나만 가져야 한다.
  - 프로퍼티는 주 생성자에서 초기화돼야 한다.
  - 다른 클래스를 상속할 수도 없고, 인라인 클래스를 다른 클래스가 상속할 수도 없다.
  - 인터페이스 상속, 메서드 정의, 계산된 프로퍼티는 제공할 수 있다.
...
#### 14.5.1 일시 중단 함수 호출
- 일시 중단 함수는 실행을 일시 중단할 수 있기 때문에 일반 코드 아무 곳에서나 호출할 수 없다.
...
- 코루틴 디스패처는 코루틴을 실행할 스레드를 결정한다.
  - 디스패처를 선택함으로써 코루틴을 특정 스레드로 제한하거나 스레드 풀에 분산시킬 수 있다.
  - 코루틴이 한 스레드에만 실행될지 여러 스레드에서 실행될지 결정할 수 있다.
- 본질적으로 코루틴은 특정 스레드에 고정되지 않는다.
#### 14.7.1 디스패처 선택
- 코루틴은 기본적으로 부모 코루틴에서 디스패철르 상속받으므로 모든 코루틴에 대해 명시적으로 디스패처를 지정할 필요는 없다.
- 디스패처의 종류
  - Default : 코루틴을 기본 환경에서 실행
    - 일반적인 작업에 사용
    - CPU 코어 수만큼의 스레드로 구성된 스레드 풀을 기반으로 한다.
    - 여러 스레드에서 코루틴이 분산돼 실행한다.
      - 멀티코어 시스템에서는 병렬로 실행될 수 있다.
    - 특별히 특정 스레드나 스레드 풀에 제한할 필요가 없는 한 대부분의 코루틴을 시작할 때는 기본 디스패처를 사용하는 것이 적합
  - Main : UI 스레드/프레임워크에서 실행
  - IO : 블로킹 IO 작업 처리
    - 이 디스패처에서 실행된 코루틴은 자동으로 확장되는 스레드 풀에서 실행
    - CPU 집약적이지 않은 작업에 적합
#### 14.7.2 코루틴 빌더에 디스패처 전달
- 코루틴 빌더 함수에 디스패처를 인자로 전달할 수 있다.
...
- 코루틴을 사용할 때는 스레드를 사용할 때와 같은 동시성 문제가 발생한다.
  - 여러 코루틴이 병렬로 동일한 데이터를 변경한다면 스레드와 마찬가지로 동기화나 잠금 처리를 해야 한다.
- 동시성 제어를 위해 코루틴은 `Mutex` 잠금을 제공한다.
  - 이를 통해 임계 영역(critical section)이 한 번에 하나의 코루틴만 실행되게 보장할 수 있다.
```kotlin
...
- 각 코루틴은 추가적인 문맥 정보를 담고 있다. 이 문맥은 `CoroutineContext`라는 형태로 제공된다.
- `CoroutineContext`는 여러 요소로 이루어진 집합이다.
  - 이 요소 중 하나는 코루틴이 어떤 스레드에서 실행될지를 결정하는 디스패처다.
  - 보통 코루틴의 생명주기와 취소를 관리하는 `Job` 객체도 포함된다.
  - 추가적인 메타데이터도 있다. (`CoroutineName`, `CoroutineExceptionHandler`)
- 현재 코루틴의 코루틴 컨텍스트를 확인하려면 어떤 일시 중단 함수 안에서든 `coroutineContext`라는 특별한 속성에 접근하면 된다.
  - 코틀린 코드에 정의된 것이 아니라, 컴파일러 고유의 기능이다. 실제 구현은 코틀린 컴파일러에 의해 특별히 처리된다.
- 코루틴 빌더나 `withContext` 함수에 인자를 전달하면 자식 코루틴의 콘텍스트에서 해당 요소를 덮어쓴다.
  - 여러 파라미터를 한 번에 덮어쓰려면 + 연산자를 사용해 결합할 수 있다.


## 15. 구조화된 동시성
- 여러 동시 작업을 처리할 때 어려운 점은 실행 중인 개별 작업을 추적하고, 더 이상 필요하지 않을 때 이를 취소하며, 오류를 제대로 처리하는 것이다.
- 구조화된 동시성(structured concurrency), 즉 애플리케이션 안에서 코루틴과 그 생애 주기의 계층을 관리하고 추적할 수 있는 기능이 코틀린 코루틴의 핵심에 내장돼 있다.
  - 수동으로 시작된 각 코루틴을 일일이 추적하지 않아도 기본적으로 작동한다.

### 15.1 코루틴 스코프가 코루틴 간의 구조를 확립한다.
- 구조화된 동시성을 통해 각 코루틴은 코루틴 스코프(coroutine scope)에 속하게 된다.
  - 코루틴 스코프는 코루틴 간의 부모-자식 관계를 확립하는 데 도움을 준다.
- `launch`와 `async` 코루틴 빌더 함수들은 사실 `CoroutineScope` 인터페이스의 확장 함수다.
  - 즉, 다른 코루틴 빌더 본문에서 `launch`나 `async`를 사용해 새로운 코루틴을 만들면 이 새로운 코루틴은 자동으로 해당 코루틴의 자식이 된다.
- 코루틴이 자식 코루틴을 인식하고 추적할 수 있다는 사실은, 부모 코루틴이 취소되면 자식 코루틴도 자동으로 취소되는 기능을 가능하게 한다.
#### 15.1.1 코루틴 스코프 생성: coroutineScope 함수
- `coroutineScope`함수
  - 새로운 코루틴을 만들지 않고도 코루틴 스코프를 그룹화할 수 있다.
  - 일시 중단 함수며, 새로운 코루틴 스코프를 생성하고 해당 영역 안에서 모든 자식 코루틴이 완료될 때까지 기다린다.
  - 활용 사례 : 동시적 작업 분해(concurrent decomposition of work)
  ```kotlin
      suspend fun computeSum() {
          println("START")
          val sum = coroutineScope {
              val a = async { someCompute() }
              val b = async { someCompute() }
              a.await() + b.await()
          }
          println("RESULT = $sum")
      }
...
- `CoroutineScope` 생성자 함수
  - 구체적인 생명주기를 정의하고, 동시 처리나 코루틴의 시작과 종료를 관리하고자 할 때 사용.
  - 새로운 독자적인 코루틴 스코프를 생성할 수 있다.
  - `coroutineScope`와 달리 이 함수는 실행을 일시 중단하지 않는다.
  - 단순히 새로운 코루틴을 시작할 때 쓸 수있는 스코프를 생성하기만 한다.
  - 하나의 파라미터만 받는다. 코루틴 스코프와 연관된 코루틴 컨텍스트이다.
  - 디스패처를 지정할 수 있다.
    - 디스패처를 지정할 때, `SupervisorJob`을 함께 사용하는 것이 좋다.
      - `SupervisorJob`을 동일한 영역과 관련된 다른 코루틴을 취소하지 않고, 처리되지 않은 예외를 전파하지 않게 해주는 특수한 `Job`이다.
#### `coroutineScope` 함수와 `CoroutineScope` 클래스
- `coroutineScope` 함수
  - 작업을 동시성으로 실행하기 위해 분해할 때 사용
  - 여러 코루틴을 시작하고, 그들이 완료할 때까지 기다리며, 결과를 계산할 수도 있다.
  - 자식들이 모두 완료될 때까지 기다리기 때문에 일시 중단 함수다.
  - 일시 중단 본문에서 자주 호출된다.
- `CoroutineScope` 클래스
  - 코루틴 클래스의 생명주기와 연관시키는 영역을 생성할 때 쓰인다.
  - 이 함수는 영역을 생성하지만 추가 작업을 기다리지 않고 즉시 반환된다.
  - 반환된 코루틴 스코프를 나중에 취소할 수 있다.
  - 클래스 프로퍼티로 코루틴 스코프를 저장할 때 주로 사용된다.
#### 15.1.3 `GlobalScope`의 위험성
- 전역 수준에 존재하는 코루틴 스코프
- `GlobalScope`를 사용하면 구조화된 동시성이 제공하는 모든 이점을 포기해야 한다.
  - 전역 범위에서 시작된 코루틴은 자동으로 취소되지 않는다.
  - 생명주기에 대한 개념도 없다.
  - 리소스 누수가 발생하거나 불필요한 작업을 계속 수행하면서 계산 자원을 낭비하게 될 가능성이 높다.
#### 15.1.4 코루틴 컨텍스트와 구조화된 동시성
- 코루틴 컨텍스트는 코루틴 간의 부모-자식 계층을 따라 상속된다.
- 새로운 코루틴을 시작할 때,
  - 자식 코루틴은 부모의 컨텍스트를 상속받는다.
  - 부모-자식 관계를 설정하는 역할을 하는 `Job` 객체를 새로 생성한다.
  - 코루틴 컨텍스트에 전달된 인자가 적용된다. (이 인자들은 상속받은 값을 덮어쓸 수 있다.)
- 따라서 디스패처를 지정하지 않고 새로운 코루틴을 시작하면, 부모 코루틴의 디스패처에서 실행한다.
- `launch`, `sync`와 마찬가지로, `coroutineScope` 함수도 자체 `Job` 객체를 갖고 부모-자식 계층 구조에 참여한다.
  - 코루틴 영역에서 `coroutineContext.job`을 통해 확인할 수 있다.

### 15.2 취소
- 취소는 코드가 완료되기 전에 실행을 중단하는 것을 의미한다.
  - 취소는 불필요한 작업을 막아준다. 이는 처리량(throughput)에 좋은 영향을 줄 수 있다.
  - 취소는 메모리나 리소스 누수를 방지한다.
  - 취소는 오류 처리에서도 중요하다.
#### 15.2.1 취소 촉발
- 코루틴 빌더 함수의 반환값(`Job`/`Deferred`)을 취소를 촉발하는 핸들로 사용할 수 있다.
  - 둘 다 `cancel`을 호출해 코루틴을 취소할 수 있다.
- 라이브러리 레벨에서 특정 조건에서 자동으로 코루틴을 취소하게도 할 수 있다.
#### 15.2.2 시간제한이 초과된 후 자동으로 취소 호출
- `withTimout`, `withTimoutOrNull` 함수는 최대 시간을 제한하면서 값을 계산할 수 있다.
- `withTimeout`은 `TimeoutCancellationException`을 발생시킨다.
#### 15.2.3 취소는 모든 자식 코루틴에게 전파된다.
- 코루틴을 취소하면 해당 코루틴의 모든 자식 코루틴도 자동으로 취소된다.
- 각 코루틴은 자신이 시작한 다른 코루틴을 알고 있기 때문에 취소할 때 스스로 자식들을 정리할 수 있다.
#### 15.2.4 취소된 코루틴은 특별한 지점에서 `CancellationException`을 던진다.
- 취소 메커니즘은 특수한 예외(`CancellationException`)을 특수한 지점에서 던지는 방식으로 작동한다.
  - 우선 일시 중단 지점이 특수한 지점이다.
  - 일반적으로 코루틴 라이브러리의 모든 일시 중단 함수는 `CancellationException` 예외가 던져질 수 있는 지점을 도입한다.
- 코루틴은 예외를 사용해 코루틴 계층에서 취소를 전파하기 때문에 이 예외를 실수로 삼켜버리거나 직접 처리하지 않도록 주의해야 한다.
#### 15.2.5 취소는 협력적이다.
- 코틀린 코루틴에 기본적으로 포함된 모든 함수는 이미 취소 가능하다.
- 코드를 직접 작성한다면 코루틴을 취소 가능하게 만들어야 한다.
- 일시 중단 지점이 있어야 취소 예외가 던져졌을 때 취소 가능하다. 즉 스스로 취소 가능하게 로직을 제공해야 한다.
  - 코드가 취소 가능한 다른 함수를 호출할 때는 자동으로 취소 가능 지점이 도입된다.
#### 15.2.6 코루틴이 취소됐는지 확인
- 코루틴의 취소 확인은 `CoroutineScope`의 `isActive` 속성을 확인한다.
  - `false`라면 코루틴은 활성 상태가 아니다. 이 경우 현재 작업을 완료하고, 획득한 리소스를 닫은 후 반환할 수 있다.
- `isActive`가 `false`일 때 명시적으로 취소를 할 수 있는 `ensureActive` 함수가 있다.
  - 더 이상 코루틴이 활성 상태가 아닐 경우 `CancellationException`을 던진다.
#### 15.2.7 다른 코루틴에게 기회를 주기: yield 함수
- `yield` 함수는 코드 안에서 취소 가능 지점을 제공할 뿐만 아니라 현재 점유된 디스패처에서 다른 코루틴이 작업할 수 있게 해준다.
- 코루틴 본문에 일시 중단 지점이 없으면 코루틴은 동기적, 순차적으로 동작하게 된다.
#### 15.2.8 리소스를 얻을 때 취소에 염두에 두기
- 리소스를 활용하여 `close()`가 필요한 작업을 할 경우, `use` 확장 함수나 `CancellationException`을 try-catch하여 finally에서 반드시 리소스를 닫아주자.


## 16. 플로우
### 16.1 플로우는 연속적인 값의 스트림을 모델링한다.
- 플로우는 시간이 지남에 따라 나타나는 값과 작업할 수 있게 해주는 코루틴 기반의 추상화다.
  - 플로우의 설계는 반응형 스트림(reactive stream)에서 영감을 받았다.
#### 16.1.1 플로우를 사용하면 배출되자마자 원소를 처리할 수 있다.
- `flow` 빌더 함수를 사용해 생성한다.
- `emit` 함수로 플로우에 원소를 추가한다.
- `collect` 함수로 플로우의 원소를 순회한다.
...
- 콜드 플로우는 `flow` 빌더 함수를 사용해 생성할 수 있다.
- 빌더 함수 블로 안에서 `emit` 함수를 호출해 플로우의 수집자에게 값을 제공한다.
- 수집자가 해당 값을 처리할 때까지 빌더 함수의 실행을 중단한다. (비동기 `return`과 비슷하게 생각할 수 있다.)
- 빌더 내부에서 `delay`와 같은 다른 일시 중단 함수를 호출할 수 있다.
- 플로우는 생성 시 비활성 상태이며, 최종 연산자(terminal operator)가 호출 돼야만 빌더에서 정의된 계산이 시작된다.
  - 콜드라고 불리는 이유는, 기본적으로 수집되기 시작할 때까지 비활성 상태이기 때문이다.
- `flow` 빌더 함수를 호출해도 실제 작업이 시작되지 않는다. 따라서 일시 중단 함수가 아닌 코틀린 코드에서도 플로우를 작성할 수 있다.
#### 16.2.2 콜드 플로우는 수집되기 전까지 작업을 수행하지 않는다.
- `collect` 함수
  - `Flow`를 실행시킨다.
    - 플로우를 수집하는 코드를 수집자(collector)라고 한다.
...
#### 구독자를 위한 값 재생
- `MutableSharedFlow`를 생성할 때 `replay` 값을 새용해 새 구독자를 위해 제공할 값의 캐시를 설정 가능하다.
  - `MutableSharedFlow<Int>(replay = 5)`
#### `sharedIn`으로 콜드 플로우를 공유 플로우로 전환
- `sharedIn` 함수를 사용하면 주어진 콜드 플로우를 공유 플로우로 변환할 수 있다.
  - 변환은 플로우의 코드가 실행되게 하므로 `shredIn`을 코루틴 안에서 호출해야 한다.
  - 두 번재 파라미터 `started`
    - `Eagerly` 플로우 즉시 시작
    - `Lazily` 첫 번째 구독자가 나타나야만 수집을 시작
...
- `MutableStateFlow(/* 초깃값 */).asStateFlow()`로 생성
  - 상태 플로우는 시간이 지남에 따라 변경될 수 있는 값을 나타내므로 생성자에게 초깃값을 제공해야 한다.
- 값을 배출하는 `emit` 대신 값을 갱신하는 `update` 함수 사용
- 현재 상태를 `value` 속성으로 접근 가능
  - 일시 중단 없이 값을 안전하게 읽을 수 있다.
#### `UPDATE` 함수로 안전하게 상태 플로우에 쓰기
- `value`는 가변 속성이다.
- 원자적으로 값을 갱신할 수 있는 `update` 함수를 제공한다.
- 람다로 새 값을 어떻게 계산해야 하는지를 받는다.
#### 상태 플로우는 값이 실제로 달라졌을 때만 값을 배출한다: 동등성 기반 통합
- `update`가 호출 되었더라도, 이전 값과 새 값이 같으면 배출이 발생하지 않는다.
#### `stateIn`으로 콜드 플로우를 상태 플로우로 변환하기
- `stateIn` 함수를 사용해 콜드 플로우를 상태 플로우로 변환할 수 있다.
  - 플로우에서 배출된 최신 값을 항상 읽을 수 있다.
  - 항상 주어진 코루틴 스코프 안에서 플로우를 시작하고, 코루틴 스코프가 취소될 때까지 구독자에게 `value` 프로퍼티를 통해 최신 값을 제공한다.
...
- 상태 플로우
  - 어떤 상태를 나타낸다.
  - 동등성 기반 통합, 즉 상태 플로우가 나타내는 값이 실제로 변경될 때만 배출된다.
  - 일반적으로 공유 플로우보다 간단한 API
#### 16.3.4 핫 플로우, 콜드 플로우, 공유 플로우, 상태 플로우: 언제 어떤 플로우를 사용할까?
- 네트워크 요청이나 데이터베이스 읽기와 같은 서비스를 제공하는 함수는 콜드 플로우를 사용해 선언한다.
  - 이를 사용하는 다른 클래스나 함수는 콜드 플로우를 직접 수집하거나 필요한 경우 이 정보를 시스템의 다른 부분에 제공하기 위해 상태 플로우나 공유 플로우로 변환할 수 있다.


## 17장. 플로우 연산자
### 17.1 플로우 연산자로 플로우 조작
- 시퀀스처럼 플로우도 중간 연산자와 최종 연산자를 구분한다.
  - 중간 연산자는 코드를 실행하지 않고 변경된 플로우를 반환한다.
  - 최종 연산자는 컬렉션, 개별 원소, 계산된 값을 반환하거나 아무 값도 반환하지 않으면서 플로우를 수집하고 실제 코드를 실행한다.

### 17.2 중간 연산자는 업스트림 플로우에 적용되고 다운스트림 플로우를 반환한다
- 중간 연산자를 플로우에 적용돼 새로운 플로우를 반환한다.
- 업스트림 플로우 = 연산자가 적용되는 플로우
- 다운스트림 플로우 = 연산자가 반환하는 플로우
  - 이 다운스트림 플로우는 또 다른 연산자의 업스트림 플로우가 될 수 있다.
  - 반환된 플로우는 콜드 상태다.
...
## 18. 오류 처리와 테스트
- 애플리케이션의 강건성을 높이기 위해 보아야 할 것은 오류 처리와 테스트이다.

### 18.1 코루틴 내부에서 던져진 오류 처리
- 일시 중단 함수나 코루틴 빌더 안에 작성한 코드도 예외를 발생시킬 수 있다.
- `launch`나 `async`의 코루틴 빌더를 try-catch로 감싸도 예외를 잡을 수 없다.
  - 대신 try-catch를 코루틴 빌더 안에 넣어야 작동한다.

### 18.2 코틀린 코루틴에서의 오류 전파
- `Job`: 코틀린 작업을 동시적으로 분해해 처리하는 경우
  - 자식 중 하나의 실패는 최종 결과를 얻을 수 없음을 의미한다.
  - 따라서 부모 코루틴도 예외로 완료되어야 한다.
  - 작업 중인 다른 자식을 더 이상 필요가 없기에 취소된다.
- `SupervisorJob`: 부모가 자식의 실행을 감독하는 경우
  - 하나의 자식이 실패해도 전체 실패로는 이어지지 않아야 한다.
  - 자식들에서 일어난 실패를 부모가 처리해야 하지만, 자식의 실패로 인해 시스템 전체가 실패하면서 멈추지 않아야 한다.
  - 감독 코루틴은 일반적으로 코루틴 계층의 최상위에 있다.
#### 18.2.1 자식이 실패하면 모든 자식을 취소하는 코루틴
- `SupervisorJob` 없이 생성된 경우 자식 코루틴에서 발생한 잡히지 않은 예외는 부모 코루틴을 예외로 완료시키는 방식으로 처리된다.
- 실패한 자식 코루틴은 자신의 실패를 부모에게 전파한다.
- 그 다음 부모의 행동은,
  - 다른 모든 자식을 취소한다.
  - 같은 예외를 발생시켜 자신의 실행을 완료한다.
  - 자신의 상위 계층으로 예외를 전파한다.
#### 18.2.2 구조적 동시성은 코루틴 스코프를 넘는 예외에만 영향을 미친다
- 코루틴 스코프를 넘지 않게 예외를 처리하면 부모&형제 코루틴에 영향을 주지 않는다.
#### 18.2.3 슈퍼바이저는 부모와 형제가 취소되지 않게 한다
- 슈퍼바이저는 자식이 실패하더라도 생존한다.
- 슈퍼바이저는 다른 자식 코루틴을 취소시키지 않으며 예외를 구조적 동시성 계층 상위로 전파하지 않는다.
- 예외가 발생하면 `CoroutineExceptionHandler`를 호출한다.
...
- `retry`를 통해 예외 발생 시, 작업 재시도를 할 수 있다.
- `retry`는 업스트림의 예외를 찾는다.
- `retry`의 람다는 `Boolean`을 반환한다.
  - 람다 실행 결과가 `true`면 재시도가 시작된다.
  - 업스트림의 플로우가 처음부터 다시 수집되면서 모든 중간 연산이 다시 실행된다.
  - 업스트림이 모두 다시 실행되기 때문에, 플로우가 부수 효과를 일으키는 경우 멱등성이 보장되게 설계하는 것이 좋다.

### 18.5 코루틴과 플로우 테스트
- 테스트에서 코루틴을 사용하려면 `runTest` 코루틴 빌더를 사용한다.
- `runBlocking`은 `delay` 같은 코드를 실제로 기다려야 한다. 이는 테스트 시간의 증가를 초래한다.
#### 18.5.1 코루틴을 사용하는 테스트를 빠르게 만들기: 가상 시간되 테스트 디스패처
- 코틀린 코루틴은 가상 시간을 사용해 테스트 실행을 빠르게 할 수 있다.
- `runTest`는 속도를 높이기 위해 특별한 테스트 디스패처와 스케줄러를 사용한다.
  - 그래서 실제로 지연 시간을 기다리지 않고 빠르게 진행시킨다.
- `runTest`의 디스패처는 단일 스레드다.
  - 모든 자식 코루틴은 동시에 실행된다. 기본적으로 테스트 코드와 병렬로 실행되지 않는다.
  - 따라서 다른 코루틴이 코드를 실행하려면 코드가 일시 중단 지점을 제공해야 한다.
- 테스트 디스패처에서 `TestCoroutineSchedular`를 통해 가상 시간을 더 세밀하게 제어할 수 있다.
  - `TestCoroutineSchedular`도 코루틴 컨텍스트다.
  - `runTest` 빌더 안에서 `TestScope`라는 특수한 스코프를 통해 접근할 수 있다.
