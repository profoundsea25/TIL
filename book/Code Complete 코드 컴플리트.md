...
- 측정은 간단한 것부터. 측정 표준화 후 개선하기

### 28.5 개발자를 사람으로 대우하기
- 통계를 살펴보면 30% 정도 프로젝트에 직접적으로 도움되지 않는 데에 시간을 사용
- 20:80 법칙 - 상위 20%의 개발자가 전체 80%의 공헌을 한다.
- 상위 10%와 하위 10% 개발자의 차이는 크다.
- 표준은 개발자들이 스스로 만들도록 한다.
- 물리적 환경(사무실)은 큰 생산성 차이를 만든다.
  - 상위 25% 능력을 가진 개발자는 더 크고 조용하고 사적인 사무실에서 일한다.

### 28.6 관리자 관리
- 관리자가 나에게 일을 주는게 아니라, 내가 관리자에게 무엇을 할지 말하라.
- 작업 수행을 위한 올바른 방법을 관리자에게 알려라. (관리자가 바뀌는 경우가 많다.)
- 관리자가 바라는 것을 처리하라. 그러나 구현 세부사항은 방해받지 말라.
- 관리자를 교육하려고 노력하라.
...
- 상향식 통합: 계층 구조의 맨 아래부터 클래스를 작성하고 통합한다.
  - 점증적 통합이라고 볼 수 있다. 저수준 클래스를 한꺼번에 추가하지 않고 한 번에 하나씩 추가하기 때문이다.
  - 고수준 시스템 인터페이스를 가장 마지막에 통합할 수 있다는 단점이 있다.
  - 통합하기 전에 전체 시스템 설계를 마쳐야 한다. 그렇지 않으면 저수준 코드의 문제점을 해결하기 위해 상위 수준 클래스를 설계해야 한다.
    - 저수준 세부 사항이 상위 수준 클래스 설계에 영향을 미치는 것은 정보 은닉과 객체지향 설계의 원칙에 어긋난다.
- 샌드위치 통합: 순수 하향식 통합과 순수 상향식 통합은 현실적으로 쉽지 않기에, 적절히 섞어서 통합한다.
  - 고수준 비즈니스 객체 클래스를 통합한 후, 저수준 클래스와 하위수준 유틸 클래스를 통합한다.
  - 중간 수준의 클래스는 마지막까지 남긴다.
- 위험 지향적인 통합: 가장 구현하기 어려운 부분을 우선 구현 후 통합
- 기능 지향적인 통합: 기능 단위로 통합
  - 독립적인 기능 단위로 추가하기 때문에 통합의 부담이 적다.
- T자형 통합 : 가장 공통되는 부분을 수직적으로 만든 후, 다른 기능들을 통합
  - 아키테처적 가정을 검증하고, 나머지 기능을 개발하기 위한 프레임워크를 제공할 시스템을 통합한다.
- 정리하자면, 통합은 견고한 절차라기 보단 경험적 법칙이다. 상황에 맞게 써야 한다.

### 29.4 일일 빌드와 스모크 테스트
- 모든 파일을 매일 컴파일하고 링크하며 실행 프로그램을 만든다. 그리고 잘 실행되는지 확인한다.
- 쉽게 매일 결함을 진단할 수 있다. 왜 작동하지 않는지 알아내기 쉽다.
- 일일 빌드와 스모크 테스트를 자동화하라.
- 빌드를 완전하게 유지하는 것을 프로젝트 최우선 순위로 두어라. 망가진 빌드는 규칙이 아니다.
- 빌드가 아무리 커도 매일 빌드할 수 있다. 윈도우 2000은 빌드에 19시간이 걸렸지만, 매일 빌드를 진행했다.
- 지속적인 통합에서의 '지속적인'은 '적어도 매일'을 의미한다.

## 30장. 프로그래밍 도구
- 해결해야 할 현실 세계 문제와 그러한 문제를 해결하기 위한 컴퓨터 사이를 연결해 줄 수 있는 사람이 바로 개발자다.


## 7부. 소프트웨어 장인정신
## 31장. 레이아웃과 스타일
- 코드의 구조를 보여주는 것이 중요하다.
  - 구조 vs 외관이라면, 구조를 더 잘 보여주는 기법을 선택하라.
- 좋은 레이아웃은 프로그램의 외견상 구조와 논리적 구조를 일치시키거나 사람과 컴퓨터가 코드를 똑같은 내용으로 이해할 수 있게 해준다는 것이 핵심이다.
- 전문가들이 프로그램의 중요한 특징을 인지하고 이해하고 기억하는 데 구조가 도움이 된다.
- 일관된 형태로 구조화하라.
- 훌륭한 개발자라면 적응하는 데 어려움이 있더라도 더 좋은 습관이 있더라면 받아들일 수 있어야 한다.
- 좋은 레이아웃의 목적은 코드의 논리적인 구조를 보여주는 것이다.
